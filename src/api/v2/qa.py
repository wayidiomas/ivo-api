...
async def _generate_qa_for_unit_sync(
    ...
    try:
        ...
        logger.info("📊 Criando embedding do Q&A...")
        try:
            ...
            if embedding_success:
                logger.info("✅ Embedding do Q&A criado com sucesso")
            else:
                logger.warning("⚠️ Falha ao criar embedding do Q&A (não afeta resultado)")
        except Exception as embedding_error:
            logger.warning(f"⚠️ Erro ao criar embedding do Q&A: {str(embedding_error)}")
        
        # 10. Manter ou atualizar status (Q&A é complementar, não muda status principal)
...
def _calculate_target_question_count(cefr_level: str, sequence_order: int) -> int:
    """Calcular número alvo de perguntas baseado no nível e sequência."""
    base_counts = {
        "A1": 5,
        "A2": 6,
        "B1": 7,
