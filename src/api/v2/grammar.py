...
async def _generate_grammar_for_unit_sync(
    ...
    try:
        ...
        # Extrair dados do grammar_params para compatibilidade com assinatura do método
        ...
        logger.info("📊 Criando embedding do grammar...")
        try:
            ...
            if embedding_success:
                logger.info("✅ Embedding do grammar criado com sucesso")
            else:
                logger.warning("⚠️ Falha ao criar embedding do grammar (não afeta resultado)")
        except Exception as embedding_error:
            logger.warning(f"⚠️ Erro ao criar embedding do grammar: {str(embedding_error)}")
        
        # 11. Atualizar lista de estratégias usadas
...
async def generate_grammar_for_unit(
    ...
    """
    Gerar estratégias GRAMMAR para unidade gramatical.
